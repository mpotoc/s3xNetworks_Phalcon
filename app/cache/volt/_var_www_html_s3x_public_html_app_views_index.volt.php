...
<html>
	<head>
		<?= $this->tag->stylesheetLink('css/normalize.css') ?>
		<?= $this->tag->stylesheetLink('css/font-awesome.css') ?>
		<?= $this->tag->stylesheetLink('css/base.css') ?>
		<?= $this->tag->stylesheetLink('css/bootstrap.css') ?>
		<?= $this->tag->stylesheetLink('css/jquery-ui.css') ?>
		<?= $this->tag->stylesheetLink('css/msgBoxLight.css') ?>
		<?= $this->tag->stylesheetLink('css/validetta.css') ?>
		<?= $this->tag->stylesheetLink('css/owl.carousel.css') ?>
		<?= $this->tag->stylesheetLink('css/owl.theme.css') ?>
		<?= $this->tag->stylesheetLink('css/owl.transitions.css') ?>
	...
	<div id="popupBottom" class="popover logon">
		<div class="popover-content">
			<?= $this->tag->form(['login', 'role' => 'form']) ?>
			<div class="inner-addon left-addon">
				<i class="glyphicon glyphicon-user"></i>
				<?= $this->tag->textField(['email', 'class' => 'form-control', 'placeholder' => 'e-mail']) ?>
			</div>
			<br>
			<div class="inner-addon left-addon">
			...
			<div id="loginsubmit">
				...
				<a id="forgotpwd" href="<?= $this->url->getBaseUri() ?>forgotPassword">Forgot your password?</a>
				<?= $this->tag->submitButton(['Log In', 'class' => 'myButton3']) ?>
			</div>
			</form>
		</div>
	</div>
	...
	<div class="modal" id="commentModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<?= $this->tag->form(['comment', 'role' => 'form', 'id' => 'outside_form']) ?>
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
					...
					<div class="modal-body">
						...
						<?= $this->tag->textArea(['comment', 'class' => 'form-control', 'rows' => '6', 'placeholder' => 'e.g., Comment about this escort', 'data-validetta' => 'required']) ?>
					</div>
					<div class="modal-footer">
						<?= $this->tag->submitButton(['Comment', 'class' => 'myButton3']) ?>
					</div>
				</form>
			</div>
		</div>
	...
	<div class="modal" id="withdrawModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<?= $this->tag->form(['withdraw', 'id' => 'outside_form']) ?>
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
					...
					<div class="modal-body">
						...
						<br><br>
						Amount to withdraw: <?= $this->tag->textField(['amount', 'class' => 'form-control', 'placeholder' => 'e.g. 100', 'data-validetta' => 'required,positive']) ?>
					</div>
					<div class="modal-footer">
						<?= $this->tag->submitButton(['Submit', 'class' => 'myButton3']) ?>
					</div>
				</form>
			</div>
		</div>
	...
	<?= $this->tag->javascriptInclude('js/jquery-1.12.4.min.js') ?>
	<?= $this->tag->javascriptInclude('js/owl.carousel.js') ?>
	<?= $this->tag->javascriptInclude('js/jquery.cookie.js') ?>
	<?= $this->tag->javascriptInclude('js/jquery-ui.js') ?>
	<?= $this->tag->javascriptInclude('js/jquery.msgBox.js') ?>
	<?= $this->tag->javascriptInclude('js/bootstrap.js') ?>
	<?= $this->tag->javascriptInclude('js/validetta.js') ?>
	<?= $this->tag->javascriptInclude('js/jquery.countdown.js') ?>
	<?= $this->tag->javascriptInclude('js/s3xnetworks.js') ?>
	<?= $this->tag->javascriptInclude('js/cycle.js') ?>
	<?= $this->tag->javascriptInclude('js/cycle2.js') ?>
	<?= $this->tag->javascriptInclude('js/liqpay.js') ?>
