...
;(function( $ ) {
  ...
  var Validetta = {}, // Plugin Class
    ...
    RMAIL = new RegExp( /^([a-z\d!#$%&'*+\-\/=?^_`{|}~\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]+(\.[a-z\d!#$%&'*+\-\/=?^_`{|}~\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]+)*|"((([ \t]*\r\n)?[ \t]+)?([\x01-\x08\x0b\x0c\x0e-\x1f\x7f\x21\x23-\x5b\x5d-\x7e\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]|\\[\x01-\x09\x0b\x0c\x0d-\x7f\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))*(([ \t]*\r\n)?[ \t]+)?")@(([a-z\d\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]|[a-z\d\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF][a-z\d\-._~\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]*[a-z\d\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])\.)+([a-z\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]|[a-z\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF][a-z\d\-._~\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]*[a-z\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])\.?$/i ),
    //RegExp for input number control method
    RNUMBER = new RegExp( /^[\-\+]?(\d+|\d+\.?\d+)$/ ),
    // RegExp for phone numbers
    RPHONE = new RegExp( /^[0-9-+]+$/ ),
    // RegExp for positive integers
    RPOSITIVE = new RegExp( /^[1-9]\d*$/),
    // RegExp for correct age
    RAGE = new RegExp( /^(1[89]|[2-9][0-9])$/),
    //
    RPRICE = new RegExp( /^[0-9]\d*$/),
  ...
  messages = {
    ...
    positive    : 'You can enter only positive integers',
    price       : 'Price for 2h and night must be positive integer or 0. If you set as 0 it will not be visible on ad!',
    ...
    minSelected : 'Minimum {count} selection required.',
    notEqual    : 'Password fields do not match.',
    different   : 'New password cannot be the same as current password.',
    creditCard  : 'Invalid credit card number.',
    packages    : 'You must select a value.'
  },
  ...
  defaults = {
    ...
    bubbleGapLeft: 5, // Right gap of bubble
  ...
  Validator = {
    ...
    // Phone check
    phone : function( tmp ) {
      return RPHONE.test( tmp.val ) || messages.phone;
    },
    // Age check
    age : function( tmp ) {
      return RAGE.test( tmp.val ) || messages.age;
    },
    // Number check
    number : function( tmp ) {
    ...
    // Integer check
    positive : function( tmp ) {
      return RPOSITIVE.test( tmp.val ) || messages.positive;
    },
    // Integer check
    packages : function( tmp ) {
      return RPOSITIVE.test( tmp.val ) || messages.packages;
    },
    // Price check
    price : function( tmp ) {
      return RPRICE.test( tmp.val ) || messages.price;
    },
